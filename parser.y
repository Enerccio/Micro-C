...
%token <token>  INC DEC PLUS MINUS MUL DIV LNEG BNEG SHIFTL SHIFTR LESS MORE LESS_EQ MORE_EQ BAND BOR OR AND 
...
%left PLUS MINUS MUL DIV SHIFTL SHIFTR LESS MORE LESS_EQ MORE_EQ BAND BOR OR AND BXOR EQ NEQ LNEG BNEG
%right ELSE "then"
...
stmt : block { $$ = $<block>1; }
	 ...
	 | identifier SET expression EOS { $$ = new BinaryOperation($<expr>1, $<expr>3, O_SET, true); }
	 ...
	 | FOR LEFT_BRACE expression EOS expression EOS expression EOS RIGHT_BRACE stmt { $$ = new ForStatement($<expr>3, $<expr>5, $<expr>7, $<stmt>9);}
...
expression : NUMBER { $$ = new Int($1); }
		   ...
		   | LEFT_BRACE identifier SET expression RIGHT_BRACE { $$ = new BinaryOperation($<expr>2, $<expr>4, O_SET); }
